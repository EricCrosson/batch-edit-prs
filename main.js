...
async function allChecksPassed(owner, repo, ref) {
    ...
    return checks.check_runs.every(check => check.conclusion === 'success' || check.conclusion === 'skipped');
}
...
async function processAssignedPullRequests() {
    try {
        ...
        for await (const { data: issues } of octokit.paginate.iterator(octokit.search.issuesAndPullRequests, {
            q: `is:pull-request is:open assignee:${username} ${pattern}`,
            per_page: 100,
        })) {
            for (const issue of issues) { // Directly iterating over issues
                if (issue.pull_request) {
                    ...
                    const allStatusChecksArePassing = await allChecksPassed(owner, repo, pr.data.head.sha);
                    console.log({ repo: `${owner}/${repo}`, title: pr.data.title, mergeable: pr.data.mergeable, allStatusChecksArePassing });

                    if (pr.data.user.login === 'renovate[bot]' && pr.data.mergeable && allStatusChecksArePassing) {
